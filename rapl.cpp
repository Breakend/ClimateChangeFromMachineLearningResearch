...
struct ResultsStorage { 
    std::vector<double> gCPU, gGPU, gMEM, gOther; 
}; 
  
typedef struct ResultsStorage Struct; 
Struct createResultsStorage(std::vector<double> gCPU, std::vector<double> gGPU, std::vector<double> gMEM, std::vector<double> gOther) 
{ 
    Struct s; 
    s.gCPU = gCPU;
    s.gGPU = gGPU;
...
static std::vector<double> gCPU_W;
static std::vector<double> gGPU_W;
static std::vector<double> gMEM_W;
static std::vector<double> gOTHER_W;
...
static void SigAlrmHandler(int aSigNum, siginfo_t* aInfo, void* aContext) {
  ...
  gCPU_W.push_back(JoulesToWatts(cores_J));
  gGPU_W.push_back(JoulesToWatts(gpu_J));
  gOTHER_W.push_back(JoulesToWatts(other_J));
  gMEM_W.push_back(JoulesToWatts(ram_J));
...
static ResultsStorage Finish() {
  ...
  return createResultsStorage(gCPU_W, gGPU_W, gMEM_W, gOTHER_W);
...
static ResultsStorage GetPowerStats(int sampleInterval_msec, int sampleCount) {
